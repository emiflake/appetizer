    ...
    pub fn get<T: LoadableResource>(&mut self, path: String) -> Result<&T, ResourceLoadError> {
        if self.resources.contains_key(&path) {
            ...
            return match self.resources.get(&path) {
                Some(resource) => resource.downcast_ref::<T>().ok_or(ResourceLoadError::TypeMismatchError{path}),
                None => unreachable!(),
            }
        }

        let loaded = T::load(&path).map_err(|_| ResourceLoadError::AcquisitionError{path: path.clone()})?; // TODO
        self.resources.insert(path.clone(), loaded);
        Ok(self.resources.get(&path).unwrap().downcast_ref::<T>().unwrap())
    }
}
...
impl LoadableResource for ImageBuffer<Rgb<u8>, Vec<u8>> {
    ...
    fn load(path: &String) -> Result<Box<Self>, Self::LoadError> {
        let mut f = File::open(path)
			.map_err(|_| format!("Could not open file of image {}	", path))?;
